...
시맨틱 세그멘테이션은 이미지의 각 픽셀에 레이블이나 클래스를 할당하는 작업입니다. 여러 종류의 세그멘테이션이 있지만, 시맨틱 세그멘테이션의 경우 같은 객체에는 서로 다른 개체들을 구별하지 않고 레이블을 할당합니다. 즉, "자동차-1"과 "자동차-2" 대신에 두 개체에 모두 "자동차" 라벨이 할당됩니다. 시맨틱 세그멘테이션은 자율주행 자동차가 보행자와 교통 정보를 식별하는 훈련, 의료 영상에서 세포와 이상을 식별하는 응용, 그리고 위성 영상을 통한 환경 변화 모니터링과 같은 다양한 실제 응용 분야에서 활용됩니다.
